...
class Pagination {
    ...
    function showPageNumbers($totalrows, $page, $limit, $maxnumberlinks = 50) {
        ...
        for ($i = max(1, $page - 10); $i <= min($totalrows / $limit, $page + $numofpages - 1); $i++) { //MODIFIED
            ...
            if ($i == $page) {
                ...
                $pagination_links .= '<div class="page-link"><a href="?page=' . $i . '&' . (isset($query_string) ? $query_string : "") . '">' . $i . '</a></div> ';
            }
    ...
    function showNext($totalrows, $page, $limit, $text = "next &raquo;") {
        ...
        if ($page < $numofpages) {
            $page++;
            $next_link = '<div class="page-link"><a href="?page=' . $page . '&' . (isset($query_string) ? $query_string : "") . '">' . $text . '</a></div>';
        } else {
            $next_link = null;
        }
    ...
    function showPrev($totalrows, $page, $limit, $text = "&laquo; prev") {
        ...
        if ($page > 1) {
            $page--;
            $prev_link = '<div class="page-link"><a href="?page=' . $page . '&' . (isset($query_string) ? $query_string : "") . '">' . $text . '</a></div>';
        } else {
            $prev_link = null;
        }
