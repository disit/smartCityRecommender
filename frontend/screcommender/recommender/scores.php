...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        $where = isset($_GET['user']) ? (" WHERE user_id = '" . $_GET['user'] . "'") : "";
        echo "SELECT COUNT(*) FROM recommender." . $config['table'] . " a " . "LEFT JOIN recommender.users b ON user_id = b.id LEFT JOIN recommender.categories c ON a.item_id = c.id " . $where;
        $totalrows = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(*) AS total FROM recommender." . $config['table'] . " a " . "LEFT JOIN recommender.users b ON user_id = b.id LEFT JOIN recommender.categories c ON a.item_id = c.id " . $where));
        ...
        if (!isset($_GET['orderby']) || trim($_GET['orderby']) == "") {
            ...
            $sql = "SELECT USER_ID FROM " . $config['table'] . $where . " LIMIT 1"; //USE ID AS THE DEFAULT SORT FIELD
        ...
        //GET DATA
        $sql = "SELECT user_id, user, item_id, c.`key` as class, preference FROM " . $config['table'] . " a " . "LEFT JOIN recommender.users b ON user_id = b.id LEFT JOIN recommender.categories c ON a.item_id = c.id " . $where . " ORDER BY $orderby $sort LIMIT $startrow,$limit";
        $result = mysqli_query($link, $sql) or die(mysqli_error());
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            foreach ($row as $field => $value) {
                ...
                if ($field == "user") {
                    echo "<td><a title=\"View user\" href=\"scores.php?user=" . $row['user_id'] . "\">" . $value . "</a></td>\n";
