...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        if (!isset($_GET['orderby']) || trim($_GET['orderby']) == "") {
            ...
            $sql = "SELECT name FROM recommender." . $config['table'] . " LIMIT 1"; //USE ID AS THE DEFAULT SORT FIELD
        ...
        //GET DATA
        $sql = "SELECT name, text, type, value, comment FROM " . $config['table'] . " ORDER BY $orderby $sort LIMIT $startrow,$limit";
        $result = mysqli_query($link, $sql) or die(mysqli_error());
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            foreach ($row as $field => $value) {
                ...
                } else if (strpos($field, 'value') !== false) {
                    echo "<td>" . printSetting($row["type"], $row["name"], $value) . "</td>\n";
                } else {
                    echo "<td>" . $value . "</td>\n";
                }
            }
